d3.layout.orbit = function () {
    ...
    function calculateNodes() {
        ...
        orbitNodes.ring = orbitSize[0] * 0.75;
        orbitNodes.depth = 0;
        ...
        function traverseNestedData(_node) {
            ...
            if (target) {
                ...
                for (var x = 0; x < target.length; x++) {
                    ...
                    target[x].parent = _node;
                    target[x].depth = _node.depth + 1;

                    target[x].x = childrenAccessor(_node)[x].parent.x + ((target[x].parent.ring / 2) * Math.sin(target[x].angle));
                    target[x].y = target[x].parent.y + ((target[x].parent.ring / 2) * Math.cos(target[x].angle));
                    target[x].x0 = target[x].parent.x;
                    target[x].y0 = target[x].parent.y;

                    target[x].deltaX = function (_x) {
                        return _x
                    }
                    target[x].deltaY = function (_y) {
                        return _y
                    }
