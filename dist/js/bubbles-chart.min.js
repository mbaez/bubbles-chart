...
function OrbitBuilder(config){TreeBuilder.call(this,config),this.rscale=0,this.center={node:[],ring:[],idx:0}}function d3Clone(node){return node.node().cloneNode(!0)}OrbitBuilder.prototype=new TreeBuilder,OrbitBuilder.prototype.constructor=OrbitBuilder,OrbitBuilder.prototype.builder=function(data){var vizId="#"+this.vizId,thiz=this,viz=document.getElementById(this.vizId);viz.innerHTML="",orbitScale=d3.scale.linear().domain([1,3]).range([3.8,1.5]).clamp(!0);var orbit=d3.layout.orbit().size([this.diameter,this.diameter]).children(function(d){return d.children}).revolution(function(d){return d.depth}).orbitSize(function(d){return orbitScale(d.depth)}),gdata=this.prepareData(data);orbit.nodes(gdata);var container=$(vizId).find("svg");container=0==container.length?d3.select(vizId).append("svg"):d3.select(vizId).select("svg"),this.center.node.length>0&&(container.node().appendChild(this.center.node[this.center.idx-1]),container.node().appendChild(this.center.ring[this.center.idx-1]));var nodes=container.selectAll("g.node").data(orbit.nodes()).enter().append("g").attr("class",function(d){var clazz="node";return d.parent||(clazz+=" center"),clazz}).attr("transform",function(d){return d.parent?"translate("+d.x0+","+d.y0+")":"translate("+d.x+","+d.y+")"}).on("mouseover",function(d){thiz.trigger("mouseover",d),d3.select(this).attr("stroke-width","5px"),thiz.createTooltip(this,d)}).on("mouseout",function(d){thiz.trigger("mouseout",d),d3.select(this).attr("stroke-width","1px"),d3plus.tooltip.remove(thiz.config.scope+"_visualization_focus")}).on("click",function(d){thiz.onClick(d3.select(this),d)});nodes.transition().duration(this.config.tree.speed).ease("linear").attr("transform",function(d){return"translate("+d.x+","+d.y+")"}).attr("T",1).each("end",function(){nodes.attr("T",0)}),this.circle(d3.selectAll("g.node")),d3.select("svg").selectAll("circle.orbits").data(orbit.orbitalRings()).enter().insert("circle","g").attr("class","ring").attr("r",function(d){return d.r}).attr("cx",function(d){return d.x}).attr("cy",function(d){return d.y}).style("fill","none").style("stroke","black").style("stroke-dasharray","2,2").style("stroke-width","1px")},OrbitBuilder.prototype.onClick=function(node,d){if(d.children&&(d.parent||d._parent)){var $center=d3.select(".node.center"),cr=$center.select("circle.shape").attr("r"),cxy=$center.attr("transform");this.animateCenterNode($center,d),this.animateRingNode(node,d,{cr:cr,cxy:cxy})}},OrbitBuilder.prototype.text=function(node,options){function isLikeWhite(c){var dc=235,rgb=d3.rgb(c);return rgb.r>=dc&&rgb.g>=dc&&rgb.b>=dc}var type="undefined"!=typeof options&&"undefined"!=typeof options.type?options.type:"text",thiz=this;return node.append(type).attr("class","wrap").style("fill",function(d){var c=thiz.config.color(d[thiz.config.label]);return c=d3plus.color.text(c),isLikeWhite(c)?thiz.config.defaultColor:c}).text(function(d){return thiz.config.format.number(d[thiz.config.size])})},OrbitBuilder.prototype.orbitRotate=function(node,pd,options){function angle(d){var yi=pd.y-(y-r),xi=pd.x-(x-r),yc=r,xc=r,dx=xi-xc,dy=yc-yi,a=Math.atan2(dy,dx)*(180/Math.PI);return 0>a?a-180:a}var ring=d3.select(".ring"),x=parseFloat(ring.attr("cx")),y=parseFloat(ring.attr("cy")),r=parseFloat(ring.attr("r"));node.attr("transform",options.cxy);var shape=node.select(".shape").attr("cx",function(d){return d.x-x}).attr("cy",function(d){return d.y-y}),thiz=this,speed=this.config.tree.speed/2;shape.transition().duration(speed).ease("cubic").delay(function(d,i){return 120*i}).attrTween("transform",function(d,i){var ai=angle(d);console.log("angulo:",ai);var i=d3.interpolate(0,ai);return function(t){return"rotate("+i(t)+")"}}).each("end",function(){var ctr=node.attr("transform");ctr=ctr.split(";"),console.log(ctr),node.select(".shape").transition().duration(speed).ease("linear").attr("cy",function(d){var cr=d.r<thiz.config.tree.minRadius?thiz.config.tree.minRadius:d.r;return y-r-cr-3*thiz.config.offset}).attr("T",1).each("end",function(){d3.select(this).attr("T",0),options.end(node,speed)})})},OrbitBuilder.prototype.animateRingNode=function(node,pd,options){function reshape(node,speed){speed=speed?speed:thiz.config.tree.speed,node.select(".shape").transition().duration(speed).ease("linear").attr("r",options.cr).attr("T",1).each("end",function(d){node.select(".shape").attr("T",0),d.children&&(d.max?d._parent&&thiz.ringDrillup(d):thiz.ringDrilldown(d))})}var thiz=this;pd.max?(options.cr=this.config.tree.minRadius,this.transition(node,function(d){return"translate ( "+2*d.x+","+d.y+")"},function(){reshape(node)})):(options.end=function(node,speed){reshape(node,speed)},this.orbitRotate(node,pd,options))},OrbitBuilder.prototype.animateCenterNode=function($center,pd){var thiz=this;if(pd.max){var tmp=thiz.center.node.pop();if(thiz.center.ring.pop(),thiz.center.idx-=1,thiz.center.idx<=0)return void(thiz.center.idx=0);this.transition(d3.select(tmp),function(d){return"translate("+pd.x+","+pd.y+")"},function(){tmp.remove()})}else this.transition($center,function(d){return"translate("+-d.r/2+","+d.y+")"},function(el,d){$center.attr("class","parent");var vizId="#"+thiz.vizId,$ring=d3.select(vizId).selectAll(".ring").attr("cx",0).attr("r",function(d){return 1.4*d.r}).attr("cy",function(d){return d.y});$ring.attr("class","parent-ring"),thiz.center.node.push(d3Clone($center)),thiz.center.ring.push(d3Clone($ring)),thiz.center.idx+=1})};;
function ListBuilder(config){UiBuilder.call(this,config),this.rscale=0,config&&this.initialize()}ListBuilder.prototype=new UiBuilder,ListBuilder.prototype.constructor=ListBuilder,ListBuilder.prototype.prepareContainer=function(){UiBuilder.prototype.prepareContainer.call(this),$("#"+this.vizId).width(this.width)},ListBuilder.prototype.prepareData=function(data){var thiz=this;this.groups=[];var tmp=d3.nest().key(function(d){return thiz.groups.indexOf(d[thiz.config.group])<0&&thiz.groups.push(d[thiz.config.group]),d[thiz.config.label]}).rollup(function(d){return d}).entries(data);return tmp.max=d3.max(thiz.groups,function(d){return d}),tmp.min=d3.min(thiz.groups,function(d){return d}),tmp.map(function(d){return d[thiz.config.label]=d.key,d[thiz.config.size]=d3.sum(d.values,function(d){return console.log(d),d[thiz.config.size]}),d}),this.prepareScale(tmp),tmp},ListBuilder.prototype.prepareScale=function(data){var len=this.groups.length;this.w=this.rowSize()*len,this.x=d3.scale.linear().range([this.rowSize(),this.w]),this.xAxis=d3.svg.axis().scale(this.x).orient("top"),this.x.domain([data.min,data.max]),this.xScale=d3.scale.linear().domain([data.min,data.max]).range([this.rowSize(),this.w])},ListBuilder.prototype.bindEvents=function(node){var thiz=this;node.on("click",function(d){thiz.trigger("click",d)}).on("mouseenter",function(d){thiz.trigger("mouseenter",d)}).on("mouseover",function(d){if(thiz.trigger("mouseover",d),thiz.createTooltip(this,d),"text"==this.nodeName){var g=d3.select(this).node().parentNode;d3.select(g).selectAll("circle").style("display","none"),d3.select(g).selectAll("text.value").style("display","block")}}).on("mouseout",function(d){if(thiz.trigger("mouseout",d),d3.select(this).attr("stroke-width","1px"),d3plus.tooltip.remove(thiz.config.scope+"_visualization_focus"),"text"==this.nodeName){var g=d3.select(this).node().parentNode;d3.select(g).selectAll("circle").style("display","block"),d3.select(g).selectAll("text.value").style("display","none")}})},ListBuilder.prototype.circle=function(g,data){var thiz=this,rscale=this.scale(data),circles=g.selectAll("g").data(data.values).enter().append("circle").attr("cx",function(d,i){return thiz.xScale(d[thiz.config.group])}).attr("cy",data.y).attr("r",function(d){return rscale(d[thiz.config.size])}).attr("fill",thiz.config.color(data[this.config.label]));return this.bindEvents(circles),circles},ListBuilder.prototype.text=function(g,data){var thiz=this;return g.selectAll("g").data(data.values).enter().append("text").attr("y",data.y).attr("x",function(d){return thiz.xScale(d[thiz.config.group])}).attr("class","value").text(function(d){return d[thiz.config.size]}).attr("fill",this.config.color(data[this.config.label])).style("display","none")},ListBuilder.prototype.rowSize=function(){return 2*(this.config.listBubble.maxRadius+this.config.listBubble.padding)},ListBuilder.prototype.scale=function(data){var thiz=this;return d3.scale.linear().domain([0,d3.max(data.values,function(d){return d[thiz.config.size]})]).range([this.config.listBubble.minRadius,this.config.listBubble.maxRadius])},ListBuilder.prototype.builder=function(data){function getY(d){return index+=1,d.y=index*thiz.rowSize(),d.y}var index=0,thiz=this;data=this.prepareData(data);var svg=d3.select("#"+this.vizId).append("svg").style("width",this.width).attr("height",this.diameter),px=this.rowSize(),node=svg.append("g").attr("transform","translate(0,"+thiz.config.listBubble.maxRadius+")");node.append("g").attr("class","x axis").call(this.xAxis);var g=svg.selectAll(".node").data(data).enter().append("g").attr("y",getY).attr("class","node"),text=g.append("text").attr("x",this.w+px).attr("y",function(d){return d.y}).attr("class","label").text(function(d){return thiz.circle(d3.select(this.parentNode),d),thiz.text(d3.select(this.parentNode),d),d[thiz.config.label]}).style("fill",function(d){return thiz.config.color(d[thiz.config.label])});this.bindEvents(text);var h=this.rowSize()*data.length+3*thiz.config.padding;$("#"+this.vizId).height(h),svg.attr("height",h)};;
function MotionBubble(config){UiBuilder.call(this,config),this.rscale=0,config&&this.initialize()}MotionBubble.prototype=new UiBuilder,MotionBubble.prototype.constructor=MotionBubble,MotionBubble.prototype.circle=function(node){},MotionBubble.prototype.prepareContainer=function(){UiBuilder.prototype.prepareContainer.call(this);var $viz=$("#"+this.vizId);$viz.width(this.width),this.center={x:this.width/2,y:this.diameter/2},this.damper=.1,this.layoutGravity=-.01},MotionBubble.prototype.buildFilter=function(){var filters=[];this.config.filters.length>0&&(filters.push({text:"Todos",value:"all"}),filters=filters.concat(this.config.filters));var container=this.headerId,$footer=$("#"+container),$toogle=$("<div></div>");$toogle.addClass("bubble-toogle"),$toogle.attr("id",container+"-toogle"),$footer.append($toogle);var thiz=this;d3plus.form().data(filters).container("#"+container+"-toogle").id("value").text("text").type("toggle").draw();$footer.find(".d3plus_toggle").on("click",function(d){var $target=$(this)[0],data=$target.__data__;"undefined"!=typeof data&&("all"!==data.value?thiz.groupAll(thiz.circles,data.value):thiz.groupAll(thiz.circles))})},MotionBubble.prototype.calculateFilterCenter=function(){this.filterCenters={};var cols=this.config.cols,p=this.config.p,xpadding=100,ypadding=50,newH=this.diameter;for(var attr in this.filters){this.filterCenters[attr]={};var len=this.filters[attr].length;cols=this.config.cols,cols=cols>len?len:cols;var nrows=parseInt(len/cols);nrows=1==nrows?2:nrows;for(var dx=(this.width-xpadding)/cols,dy=(this.diameter-ypadding)/nrows,idx=1,i=0;len>i;i++){var strTmp=this.filters[attr][i],row=parseInt(i/cols),y=dy*(row+1);y=row==nrows?y-dy*p:y,idx=idx>cols?1:idx;var x=dx*idx;x=1==idx?dx*(1-p):x,x=idx==cols?x-dx*p:x,newH=y,this.filterCenters[attr][strTmp]={x:x,y:y,dx:dx,dy:dy,row:row,idx:idx,nrows:nrows},idx+=1}}var $viz=$("#"+this.vizId),$svg=$viz.find("svg");$viz.height(newH+2*ypadding),$svg.height(newH+2*ypadding)},MotionBubble.prototype.roolup=function(v,sample){var data={},thiz=this;for(var attr in sample){this.roolupFilters(v,attr);for(var i=0;i<v.length;i++)attr==this.config.time?data[attr]=v.map(function(d){return-1==thiz.timeArray.indexOf(d[attr])&&thiz.timeArray.push(d[attr]),d[attr]}):attr==this.config.size?data[attr]=d3.sum(v,function(d){return d[attr]}):data[attr]=v[0][attr]}return data},MotionBubble.prototype.buildNodes=function(data,filter){var thiz=this;UiBuilder.prototype.buildNodes.call(this,data,filter);var darray=data,max=d3.max(darray,function(d){return d[thiz.config.size]}),min=d3.min(darray,function(d){return d[thiz.config.size]});min=min==max?0:min;var nChilds=darray.length-10;nChilds=5>nChilds?5:nChilds,nChilds=nChilds>30?30:nChilds;var maxR=this.width/nChilds;return this.rscale=d3.scale.pow().exponent(.5).domain([min,max]).range([this.config.bubble.minRadius,maxR]),darray.forEach(function(d){d.x=Math.random()*thiz.width,d.y=Math.random()*thiz.diameter,d.value=d[thiz.config.size],d.r=thiz.rscale(d.value)}),darray=darray.sort(function(a,b){return b[thiz.config.size]-a[thiz.config.size]}),this.calculateFilterCenter(),darray},MotionBubble.prototype.builder=function(data){d3.select("#"+this.vizId).html("");var svg=d3.select("#"+this.vizId).append("svg").attr("width",this.width).attr("height",this.diameter);this.buildFilter();var thiz=this;this.nodes=this.buildNodes(data),this.circles=svg.selectAll("circle").data(this.nodes).enter().append("circle").attr("class","node").attr("r",0).attr("cx",function(d){return d.x}).attr("cy",function(d){return d.y}).attr("fill",function(d){return thiz.config.color(d[thiz.config.label])}).attr("stroke-width",1).attr("stroke",function(d){var c=thiz.config.color(d[thiz.config.label]);return d3plus.color.legible(c)}),this.bindMouseEvents(this.circles),this.circles.transition().duration(2e3).attr("r",function(d){return d.r}),this.start(),this.groupAll(this.circles)},MotionBubble.prototype.charge=function(d){return-Math.pow(d.r,2)/8},MotionBubble.prototype.start=function(){this.force=d3.layout.force().nodes(this.nodes).size([this.width,this.diameter])},MotionBubble.prototype.tickNodes=function(e,moveCallback){this.circles.each(moveCallback(e.alpha)).attr("cx",function(d){return d.x}).attr("cy",function(d){return d.y})},MotionBubble.prototype.groupAll=function(nodes,filter){var thiz=this;thiz.hideFilters(),this.force.gravity(this.layoutGravity).charge(this.charge).friction(.9).on("tick",function(e){thiz.tickNodes(e,function(alpha){return"undefined"!=typeof filter?thiz.moveTowardsFilter(alpha,filter):thiz.moveTowardsCenter(alpha)})}),"undefined"!=typeof filter&&this.displayFilters(filter),this.force.start()},MotionBubble.prototype.moveTowardsCenter=function(alpha){var thiz=this;return function(d){return d.x=d.x+(thiz.center.x-d.x)*(thiz.damper+.02)*alpha,d.y=d.y+(thiz.center.y-d.y)*(thiz.damper+.02)*alpha}},MotionBubble.prototype.moveTowardsFilter=function(alpha,filter){var thiz=this;return function(d){var target=thiz.filterCenters[filter][d[filter]];return d.x=d.x+(target.x-d.x)*(thiz.damper+.02)*alpha*1.1,d.y=d.y+(target.y-d.y)*(thiz.damper+.02)*alpha*1.1}},MotionBubble.prototype.displayFilters=function(attr){var thiz=this,data=d3.keys(this.filterCenters[attr]);d3.select("#"+this.vizId).select("svg").selectAll(".filters").data(data).enter().append("text").attr("class","filters").attr("x",function(d){var dt=thiz.filterCenters[attr][d],x=1==dt.idx?.5*dt.dx:dt.x;return x=dt.idx==thiz.config.cols?dt.x+.3*dt.dx:x}).attr("y",function(d){var dt=thiz.filterCenters[attr][d],y=0==dt.row?.5*dt.dy:dt.y+.3*dt.dy;return y=dt.row==dt.nrows?dt.y+.5*dt.dy:y}).attr("text-anchor","middle").text(function(d){return thiz.config.format.text(d)})},MotionBubble.prototype.hideFilters=function(){d3.select("#"+this.vizId).selectAll(".filters").remove()};;
